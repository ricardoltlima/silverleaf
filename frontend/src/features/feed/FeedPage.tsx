...
const FEED_QUERY_KEY = ["feed", "home"];
...
export function FeedPage() {
  ...
  const appendComment = useCallback(
    (postId: number, comment: FeedComment) => {
      queryClient.setQueryData<FeedPageResponse>(FEED_QUERY_KEY, (current) => {
  ...
  const submitPost = (event: FormEvent) => {
    ...
    createPostMutation.mutate({ text, media: queuedMedia });
  };
